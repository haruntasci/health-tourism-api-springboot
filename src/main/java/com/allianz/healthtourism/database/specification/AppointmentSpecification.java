    ...
    public Predicate toPredicate(Root<Appointment> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        ...
        for (SearchCriteria criteria : criteriaList) {
            ...
            if (criteria.getOperation().equals(OperationTypeEnum.GREATER_THAN)) {
                ...
                if (criteria.getKey().equals("doctor")) {
                    Join<Appointment, Doctor> joinDoctor = root.join("doctor");
                    predicate = criteriaBuilder.equal(joinDoctor.get("email"), criteria.getValue());
                } else if (criteria.getKey().equals("patient")) {
                    Join<Appointment, Patient> joinPatient = root.join("patient");
